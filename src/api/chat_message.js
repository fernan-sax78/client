...
export class ChatMessage {
    async getLastMessage(token , chatId){
        try {
            const url = `${ENV.API_URL}/${ENV.ENDPOINTS.CHAT_LAST_MESSAGE}/${chatId}`;
            const params = {
                headers : {
   ...
   async getTotalMessage(token , chatId){
        try {
            const url = `${ENV.API_URL}/${ENV.ENDPOINTS.CHAT_TOTAL_MESSAGE}/${chatId}`;
            const params = {
                headers : {
                    Authorization : `Bearer ${token}`
                }
